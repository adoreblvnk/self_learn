...
int main()
{
    ...
    // NOTE: braced initialisation forces the data to conform to the type, & throws an error if not.
    // NOTE: assignment & functional initialisation attempts to convert the data, which can cause unforeseen errors later on.
    int dogCount{};    // preferred: braced initialisation.
    int catCount = 34; // assignment initialisation.
    int chickenCount(23);
    int animalCount{dogCount + catCount + chickenCount}; // expression allowed in braces.
    std::cout << "animal count: " << animalCount << std::endl;
    std::cout << "sizeof animalCount: " << sizeof(animalCount) << " bytes" << std::endl;
    ...
    std::cout << "Size of deduceFraction: " << sizeof(deduceFraction) << " bytes" << std::endl;
